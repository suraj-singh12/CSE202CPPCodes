...
public:
    ...
    // both are friends of the Complex class
    // friend is the keyword to declare them as friend
    // Complex is the return type because they will be returning an object of Complex type
    // Complex const & is in argument, i.e. object of Complex type, and constant(not modifiable by the function)
    // and taken by reference (not copied).
    friend Complex operator+(Complex const &, Complex const &);
    friend Complex operator-(Complex const &, Complex const &);
    // Note: A friend function can access the private data members of a class too
};
Complex operator+(Complex const &c, Complex const &d){
    Complex c1;
    c1.real = d.real + c.real;
    c1.imag = d.imag + c.imag;
    return c1;
}
Complex operator-(Complex const &c, Complex const &d){
    Complex c1;
